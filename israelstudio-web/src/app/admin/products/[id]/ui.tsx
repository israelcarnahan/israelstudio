  ...
  return (
    <div className="grid gap-6 max-w-2xl">
      ...
      <div className="grid gap-2">
        ...
        <Uploader
          onDone={(im) =>
            setImages((prev) => [...prev, { url: im.url, order: prev.length }])
          }
        />
          ...
          {images.map((im, i) => (
            ...
            >
              ...
              <div className="absolute bottom-2 left-2 bg-black bg-opacity-50 text-white text-xs px-2 py-1 rounded">
                {im.order === 0 ? "Main Image" : `Image ${(im.order ?? 0) + 1}`}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex gap-3 flex-wrap">
          ...
          onClick={async () => {
            ...
            try {
              ...
              if (r.ok) {
                ...
                if (r.status === 409) {
                  alert(
                    "A product with a similar title already exists. Please choose a different title."
                  );
                } else if (errorData.error === "validation_error") {
                  alert(
                    `Validation error: ${JSON.stringify(errorData.issues)}`
                  );
                } else {
        ...
        <form
          ...
          onSubmit={async (e) => {
            ...
            await fetch(e.currentTarget.action, { method: "DELETE" });
            window.location.href = "/admin/products";
          }}
        >
